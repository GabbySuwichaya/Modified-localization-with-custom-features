...
def match_features(images, paths, args):
    ...
    for raw_pair in tqdm(raw_pairs, total=len(raw_pairs)):
        ...
        features_path1 = os.path.join(paths.feat_path, '%s.%s' % (image_name1, args.method_name))
        features_path2 = os.path.join(paths.feat_path, '%s.%s' % (image_name2, args.method_name))
        ...
        if (images.get(image_name1, -1) == -1) or (images.get(image_name2, -1) == -1) :
            print("Skip the pair %s and %s" % (image_name1, image_name2) )

            if (images.get(image_name1, -1) == -1) and not(images.get(image_name2, -1) == -1):
                print("Specifically %s is not in the db.db" % (image_name1) )

            elif (images.get(image_name2, -1) == -1) and not(images.get(image_name1, -1) == -1):
                print("Specifically %s is not in the db.db" % (image_name2) )

            else:
                print("Specifically both %s and %s are not in the db.db" % (image_name1, image_name2) )
...
if __name__ == "__main__":    
    ...
    parser.add_argument('--feat_path', required=True, help='Path to the extract features')
    ...
    paths.feat_path           = args.feat_path
    ...
    if args.matching_only == False:
      ...
      print("Create a copy of the dummy database...")
      ...
      print("Finish copying the dummy database...")
